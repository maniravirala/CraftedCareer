...
const ResumeTemplate = () => {
  ...
  const onSortTemplateItems = (event) => {
    ...
    let newItems = [...templateItems];
    newItems.splice(oldIndex, 1);
    newItems.splice(newIndex, 0, templateItems[oldIndex]);
    updateTemplateOrder({
      template2: newItems.map((item, index) => ({
        id: index + 1,
  ...
  return (
    <div>
      <DndContext
        ...
        onDragEnd={onSortTemplateItems}
        modifiers={[restrictToVerticalAxis, restrictToParentElement]}
      >
        <SortableContext items={templateItems}>
          {templateItems.map((item, index) => (
            <SortableItem key={item.key} id={item.id}>
              {item.component}
            </SortableItem>
          ))}
        </SortableContext>
